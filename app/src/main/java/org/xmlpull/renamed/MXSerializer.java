...
/**
 * Implementation of XmlSerializer interface from XmlPull V1 API. This implementation is optimzied
 * for performance and low memory footprint.
 *
 * <p>Implemented features:
 *
 * <ul>
 *   <li>FEATURE_NAMES_INTERNED - when enabled all returned names (namespaces, prefixes) will be
 *       interned and it is required that all names passed as arguments MUST be interned
...
public class MXSerializer implements XmlSerializer {
  ...
  /**
   * For maximum efficiency when writing indents the required output is pre-computed This is
   * internal function that recomputes buffer after user requested chnages.
   */
    ...
    for (int i = namespaceEnd - 1; i >= 0; --i) {
      if (namespace == namespaceUri[i]) {
        ...
        for (int p = namespaceEnd - 1; p > i; --p) {
          if (prefix == namespacePrefix[p]) continue; // too bad - prefix is redeclared with
          // different namespace
        }
        return prefix;
      }
  ...
  private String generatePrefix(String namespace) {
    ...
    while (true) {
      ...
      final String prefix =
          autoDeclaredPrefixes < precomputedPrefixes.length
  ...
  public XmlSerializer attribute(String namespace, String name, String value) throws IOException {
    if (!startTagIncomplete) {
      throw new IllegalArgumentException(
          "startTag() must be called before attribute()" + getLocation());
    }
    // assert setPrefixCalled == false;
    if (attrNewLine && doIndent) {
      writeIndent();
  ...
  protected void closeStartTag() throws IOException {
    if (finished) {
      throw new IllegalArgumentException(
          "trying to write past already finished output" + getLocation());
    }
    if (seenBracket) {
      seenBracket = seenBracketBracket = false;
    }
    if (startTagIncomplete || setPrefixCalled) {
      ...
      if (!startTagIncomplete) {
        throw new IllegalArgumentException(
            "trying to close start tag that is not opened" + getLocation());
      }
